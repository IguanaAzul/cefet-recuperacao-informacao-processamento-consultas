...
class ProcessingTest(unittest.TestCase):
    ...
    def test_count_top_n_relevant(self):
        arr_lists = [
            [1, 2, 3, 4, 5, 30, 23, 234, 32, 32, 3, 2, 10, 20],
            [-1, -2, -2],
            [14, 23, 21, 1, 10, 20, 4],
            [],
        ]
        set_relevantes = {1, 4, 5, 10, 20}
        resp_esperada_por_top_n = {
            1: [1, 0, 0, 0],
            3: [1, 0, 0, 0],
            5: [3, 0, 2, 0],
            14: [5, 0, 4, 0],
    ...
    def test_precision_recall(self):
        ...
        self.assertAlmostEqual(
            precisao, precisao_esperada, places=2, msg=f"Valor incorreto de precisão"
        )
        self.assertAlmostEqual(
            revocacao, revocacao_esperada, places=2, msg=f"Valor incorreto de revocação"
        )
    ...
    def check_terms_index(self, response: Mapping, expected_response: Mapping):
        ...
        self.assertEqual(
            len(set_faltando),
            0,
            f"Os seguintes termos deveriam ter sido retornados: {set_faltando}",
        )
        self.assertEqual(
            len(set_sobrando),
            0,
            f"Os seguintes termos não deveriam ter sido retornados: {set_sobrando}",
        )
    ...
    def check_terms_occur(self, response: Mapping, expected_response: Mapping):
        ...
        if type(list(response.values())[0]) != list:
            ...
            map_expected_resp_list = {
                term: [occur] for term, occur in expected_response.items()
            }
        else:
        ...
        for term, lst_occur_resp in map_resp_list.items():
            for occur_resp in lst_occur_resp:
                ...
                for expected_occur in map_expected_resp_list[term]:
                    if (
                        expected_occur.term_id == occur_resp.term_id
                        and expected_occur.doc_id == occur_resp.doc_id
                    ):
                        self.assertEqual(
                            expected_occur.term_freq,
                            occur_resp.term_freq,
    ...
    def test_get_occurence_list_per_term(self):
        ...
        arr_expected_response = [
            ...
            {
                "vocês": [
                    TermOccurrence(2, voces_id, 3),
                    TermOccurrence(3, voces_id, 1),
                ],
