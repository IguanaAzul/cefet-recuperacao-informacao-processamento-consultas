...
class IndexerTest(unittest.TestCase):
    def test_indexer(self):
        ...
        self.assertTrue(
            len(sobra_expected) == 0 and len(sobra_vocab) == 0,
        ...
        dic_expected = {
            111: TermOccurrence(111, 2, 1),
            100102: TermOccurrence(100102, 2, 2),
        }
        for occur in lst_occur:
            self.assertTrue(
                type(occur.doc_id) == int, f"O tipo do documento deveria ser inteiro"
            )
            self.assertTrue(
                occur.doc_id in dic_expected,
                f"O docid número {occur.doc_id} não deveria existir ou não deveria indexar o termo 'cas'",
            )
            self.assertEqual(
                dic_expected[occur.doc_id].term_freq,
                occur.term_freq,
