...
class StructureTest(unittest.TestCase):
    ...
    def test_document_count_with_term(self):
        self.assertEqual(
            2,
            self.index.document_count_with_term("casa"),
            f"Casa apareceu em dois documentos",
        )
        self.assertEqual(
            3,
            self.index.document_count_with_term("vermelho"),
            f"Vemelho apareceu em dois documentos",
        )
        self.assertEqual(
            1,
            self.index.document_count_with_term("verde"),
            f"Verde apareceu em dois documentos",
        )
        self.assertEqual(
            0,
            self.index.document_count_with_term("cinza"),
    ...
    def occur_list_test(self, index):
        ...
        for term, dic_doc_freq in dict_expected_index.items():
            ...
            for occur in list_occur:
                ...
                self.assertTrue(
                    doc_id in dict_expected_index[term],
                    f"O termo {term} não ocorre no documento {doc_id}",
                )
                self.assertEqual(
                    dict_expected_index[term][doc_id],
                    frequency,
                    f"Erro o termo {term} deveria ocorrer {dict_expected_index[term][doc_id]}x no documento {doc_id} e não {frequency}",
                )
            self.assertEqual(
                len(dic_doc_freq.keys()),
                len(list_occur),
